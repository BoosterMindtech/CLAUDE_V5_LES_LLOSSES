...
# ══ ADMIN: CONFIGURACIÓN ═════════════════════════════════════════════════════
    ...
    # ── Acceso técnico con contraseña ────────────────────────────────────────
    TECH_PASS = "BoosterMindtech"
    if "tech_auth" not in st.session_state:
        st.session_state["tech_auth"] = False

    if not st.session_state["tech_auth"]:
        st.info("🔒 Esta sección es de uso exclusivo para el técnico instalador.")
        with st.form("ftech_login"):
            pwd_tech = st.text_input("Contraseña técnico", type="password")
            if st.form_submit_button("Acceder"):
                if pwd_tech == TECH_PASS:
                    st.session_state["tech_auth"] = True
                    st.rerun()
                else:
                    st.error("Contraseña incorrecta.")
    else:
        st.success("🔓 Acceso técnico activo")
        if st.button("🔒 Cerrar sesión técnico", key="btn_tech_logout"):
            st.session_state["tech_auth"] = False
            st.rerun()
